<template>
  <div id="login">
    ...
    <div id="loginBox">
      <h4 class='logintest'>注&emsp;册</h4>
      <el-form
        :model="loginForm"
        :rules="loginRules"
        ref="loginForm"
  ...
  data() {
    return {
      ...
      loginRules: {
        ...
        ensure: [{ required: true, message: "请输入确认密码", trigger: "blur" }]
      }
    };
  },
  methods: {
    ...
    submitForm() {
        const _this=this
        this.$axios.post('/api/signup',this.loginForm).then(function(resp){
                    ...
                    if(resp.data['status']=='success'){
                        ...
                        _this.$alert(resp.data['status'], '警告', {
                        confirmButtonText: '确定',
                        callback: action => {
